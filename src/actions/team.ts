...
export async function changeMemberStatus(status: TeamMemberStatus, teamId: number, memberId: number, membershipId: number) {
...
export async function changeMemberRole(role: TeamRoles, teamId: number, memberId: number, membershipId: number) {
  const teamMemberExists = await db.teamMember.findFirst({ where: { teamId, userId: memberId } })
  if (teamMemberExists) {
    const team = await db.team.findUnique({ where: { id: teamId }, select: { name: true } })
    await notify(
...
export async function inviteMembers(teamId: number, users: User[]) {
  if (users.length > 0) {
    ...
    users.forEach(async (invitation) => {
      ...
      const findInvitation = await db.teamInvite.findMany({ where: { teamId, userId: invitation.id } })
  ...
  return {
    message: 'Selected users has been invited, Notice that if you have invited them before you won\'t be able to invite twice!',
    status: 200
  }
}
...
export async function directAddMember(code: string, teamId: number, userId: number) {
  ...
  const user = await db.user.findUnique({ where: { id: userId }, select: { id: true, directCode: true } })
