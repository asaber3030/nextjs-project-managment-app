...
export const OnePersonalProject = ({ project, projectId }: Props) => {
  ...
  return (
    ...
    <div key={`project-view-idx-${project.id}`} className='p-4 border rounded-md shadow-sm py-4 h-fit bg-white'>
      ...
      <Link href={''} className='block mx-auto text-xl mb-1 font-semibold text-center'>{project.name}</Link>
      ...
      <ul className="pt-2">
        ...
        <li className='flex justify-between items-center py-2 text-sm'>
          <span className='flex gap-2 items-center font-medium text-xs'><Calendar className='size-4' /> Created In</span>
        ...
        <li className='flex justify-between items-center py-2 pb-0 text-sm'>
          <span className='flex gap-2 items-center font-medium text-xs'><Calendar className='size-4' /> Last Update In</span>
