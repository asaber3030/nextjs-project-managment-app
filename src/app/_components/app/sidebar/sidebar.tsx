...
import { CheckCheck, CheckCircle, Cog, DollarSign, Folder, LucideHome, User, UserPlus, Users } from "lucide-react";
...
export const UserSidebar = ({ subscription, countTasks, stats }: Props) => {
  ...
  return (
    <aside className='bg-navbar hidden flex-col xl:flex xl:justify-between h-full xl:fixed left-0 top-0 w-[345px] border-r border-r-secondaryMain'>
      ...
      <div>
        ...
        <section className='p-2 pr-4'>
          ...
          <SidebarBadge label='Assigned Tasks' icon={CheckCircle} url={route.assignedTasks()}>
            {countTasks > 0 && (
              <Badge variant='outlinePrimary' className='text-xs rounded-full'>{countTasks} pending</Badge>
            )}
          </SidebarBadge>

          <SidebarLink label='Teams' icon={Users} url={route.teams()}>
            <CreateTeamButton className='border-none w-full hover:bg-secondary text-left justify-start transition-all bg-transparent' label='Create Team' />
          </SidebarLink>
          ...
          <SidebarLink label='Personal Tasks' icon={CheckCheck} url={route.personalTasks()}>
            <CreatePersonalTask label="Create" className='bg-transparent w-full rounded-md text-sm flex gap-2 p-1 px-2 h-8 justify-start transition-all hover:bg-accent' iconClassName="size-4" />
          ...
          <SidebarLink label='Account' icon={User} url={route.account()}>
            <DropdownMenuItem onClick={() => router.push(route.account('settings'))}>Change Password</DropdownMenuItem>
            <DropdownMenuItem onClick={() => router.push(route.account('picture'))}>Change Profile picture</DropdownMenuItem>
            <DropdownMenuItem onClick={() => router.push(route.account('teams'))}>Teams</DropdownMenuItem>
            <DropdownMenuItem onClick={() => router.push(route.account('projects'))}>Projects</DropdownMenuItem>
          </SidebarLink>

          <SidebarLink label='Subscriptions' icon={DollarSign} url={route.account('subscriptions')} />
